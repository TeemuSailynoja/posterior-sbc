...
# Compute initial values using Pathfinder
if (file.exists(file.path(stan_fit_dir, "pth0_pth.RDS"))) {
  pth0 <- readRDS(file.path(stan_fit_dir, "pth0_pth.RDS"))
} else {
  pth0 <- model_pred$pathfinder(
    data = list(
  ...
  if (save_stan_fit) {
    pth0$save_object(file.path(stan_fit_dir, "pth0_pth.RDS"))
  }
}

# Obtain posterior approximation with NUTS
if (file.exists(file.path(stan_fit_dir, "fit0_pth.RDS"))) {
  fit0 <- readRDS(file.path(stan_fit_dir, "fit0_pth.RDS"))
} else {
  ...
  if (save_stan_fit) {
    fit0$save_object(file.path(stan_fit_dir, "fit0_pth.RDS"))
  }
}
...
stan_backend_pth <- SBC_backend_cmdstan_sample(
  ...
  init_factory = \(stan_data) {
    cmdstanr::cmdstan_model("models/lotka_volterra_postSBC.stan")$pathfinder(
      data = stan_data,
      num_paths = 10,
      single_path_draws = 40,
      draws = 400,
      ...
      max_lbfgs_iters = 100,
      refresh = 0,
      seed = as.integer(stan_data$y_init2[1] + stan_data$y_init2[2])
    )
  }
)
...
results_post_sbc <- compute_SBC(
  ...
  cache_location = file.path(
    cache_dir,
    paste(
      "results-posterior_sbc_init-n_iter_",
      n_sbc_iter,
      "_seed_",
      seed,
      sep = ""
