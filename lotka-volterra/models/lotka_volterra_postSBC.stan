...
data {
  ...
  array[2] real y_init2;            // initial measured populations
  array[N,2] real <lower = 0> y;   // measured populations
  array[N,2] real <lower = 0> y2;   // measured populations
}
parameters {
...
model {
  ...
  for (k in 1:2) {
    ...
    y_init2[k] ~ lognormal(log(z_init[k]), sigma[k]);
    y2[ , k] ~ lognormal(log(z[, k]), sigma[k]);
  }
}
...
generated quantities {
  ...
  for (k in 1:2) {
    ...
    loglik += lognormal_lpdf(y_init2[k]|log(z_init[k]), sigma[k]);
    loglik += lognormal_lpdf(y2[ , k]|log(z[, k]), sigma[k]);
  }
}
